...
while True:
    ...
    for subject in subjects:
            ...
            if flag >= frame_check:
                ...
                if alert_timer >= 5:  
                    ...
                    with app.app_context():
                        Drowsy = drowsy(driver_id=3, driver_name="Kaustubh", v_start=v_started,v_stop=now,time=now,d_type="eyes",d_total_time=5,location=location.address)
        ...
        if mar > thresh_mar:  # Check if mouth aspect ratio indicates yawning
            ...
            now=datetime.now();
            with app.app_context():
                Drowsy = drowsy(driver_id=3, driver_name="Kaustubh", v_start=v_started,v_stop=now,time=now,d_type="yawn",d_total_time=5,location=location.address)
                db.session.add(Drowsy)
                db.session.commit()
    ...
    if key == ord("a"):
        ...
        now=datetime.now();
        with app.app_context():
            Drowsy = drowsy(driver_id=3, driver_name="Kaustubh", v_start=v_started,v_stop=now,time=now,d_type="Accident",d_total_time=0,location="sangamner")
            db.session.add(Drowsy)
            db.session.commit()
